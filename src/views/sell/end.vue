    ...
    <el-table :data="list" style="width: 100%" @expand-change="detail" class="table">
      ...
      <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" @click="endOrder(scope.row)" class="button">销售单了结</el-button>
          </template>
        </el-table-column>
    </el-table>
     <el-pagination
...
export default {
  ...
  methods: {
    ...
    queryList(payType) {
      this.tab=payType
      this.$axios
        .get("/api/main/sell/somain/show?type=4&payType="+payType)
        .then(response => {
    ...
    handleSizeChange(val) {
      ...
      handleCurrentChange(val) {
        // console.log(`当前页: ${val}`);
        this.$axios.get("/api/main/sell/somain/show?type=4&payType="+this.tab+"&page="+val).then(response => {
            this.list = response.data.list
        })
      },
    //展开显示销售单明细
    ...
    endOrder(row){
        this.$axios
        .get("/api/main/sell/somain/end?soId="+row.soId+"&payType="+this.tab).then(response => {
          if (response.data.code == 2) {
            ...
            return this.$message.error('销售单了结失败');
          }
        });
    }
    },
