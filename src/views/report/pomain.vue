<template>
  <div>
    ...
    <el-table :data="detailList" stripe style="width:95%" class="el-table">
      ...
      <el-table-column prop="poId" label="采购单编号" width="130px"></el-table-column>
      ...
      <el-table-column prop="stockUser" label="创建用户"></el-table-column>
      ...
      <el-table-column prop="status" label="采购单状态"></el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    queryData() {
      ...
      this.$axios
        .get("/api/main/report/pomain/main?time=" + this.time)
        .then(response => {
     ...
     handleSizeChange(val) {
      ...
      handleCurrentChange(val) {
        // console.log(`当前页: ${val}`);
        this.$axios.get("/api/main/report/pomain/main?time=" + this.time+"&page="+val).then(response => {
             this.mainList = response.data;
            this.detailList = response.data.details.list;
        })
      },
